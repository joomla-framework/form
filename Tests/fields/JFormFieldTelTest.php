...
class JFormFieldTelTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testGetInput()
	{
		...
		$xml = new SimpleXMLElement('<field type="tel" id="myId" name="myName" />');
		$this->assertThat(
		...
		$xml = new SimpleXMLElement('<field type="tel" id="myId" name="myName" size="0" maxlength="0" class="foo bar" readonly="true" disabled="true" onchange="barFoo();" />');
		$this->assertThat(
			$field->setup($xml, 'aValue'),
			$this->isTrue(),
			'Line:' . __LINE__ . ' The setup method should return true.'
